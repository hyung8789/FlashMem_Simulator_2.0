...
4) Search Algorithm improvement for finding empty sector(offset) in a block<br>
: Based on half of the block, if corresponding offset is recorded, all prior offsets are recorded.(don't care whether it's valid or not), so search from that location in sequential order. else, search sequentially from 0 Offset.<br>
<br></br>
