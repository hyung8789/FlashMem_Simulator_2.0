...
: <del>Based on half of the block, if corresponding offset is recorded, all prior offsets are recorded.(don't care whether it's valid or not), so search from that location in sequential order. else, search sequentially from 0 Offset.</del>Divide and conquer
<br>
<br></br>
